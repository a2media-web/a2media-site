...
/* C-1-3 — Cards live in panel, animate in via IntersectionObserver as the
 * panel area approaches viewport. Sticky shows fail → success heading.
 * Cards arrive once, with stagger.
 */

import { useEffect, useRef, useState } from "react";
import { STUDIES, TICKER, range } from "../StoryScroll/_shared";
import styles from "./StoryScrollV1Approach.module.css";

export default function StoryScrollV1Approach() {
  ...
  useEffect(() => {
    ...
    const io = new IntersectionObserver(
      (entries) => {
        for (const e of entries) if (e.isIntersecting) setCardsIn(true);
      },
      { threshold: 0.15 },
    );
    io.observe(el);
      ...
      <div ref={morphRef} className={styles.morphArea}>
        <div className={styles.morphSticky}>
          ...
          >
            ...
            <p className={styles.scrollHint}>↓ The receipts</p>
          </div>
        </div>
      </div>
        ...
        <div className={styles.panelInner}>
          <div className={styles.panelHead}>
            ...
            <p className={styles.panelSub}>
              Three teams. Three stories. Same engine.
            </p>
          </div>

          <div ref={cardsRef} className={styles.cardRow}>
            {STUDIES.map((s, i) => {
              ...
              return (
                <button
                  key={s.id}
                  type="button"
                  className={`${styles.card} ${isActive ? styles.cardActive : ""} ${cardsIn ? styles.cardIn : ""}`}
